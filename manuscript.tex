...
\ead{hawlader.almamun@uwa.edu.au}
%% \ead[url]{home page}
%% \fntext[label2]{}
%% \cortext[cor1]{}
...
%   computational burden induced by the combinatorial number of tests that have to be performed. Random Forests models were
%   widely used in a variety of disciplines including epistatic interactions. Although Random Forests models account for
%   variable interactions, it does not explicitly detect variable interactions. And most of the Random Forests based
%   methods to detect interactions relay on some kind of variable importance measures that suffer when the interacting
%   variables have very small or no marginal effects. We proposed a Random Forests based method that is not based on
...
% obtained from the constituent models.  In a Random Forests, \textit{m} random samples (with replacement) are selected by
...
% implementation of Random Forests algorithm namely the number of decision trees (\texttt{num.trees}) and the number of
...
% interactions. First, we built a Random Forests and for each pair of SNPs we count the total number of times any SNP pair
...
% at least two (\textit{t}=2) appearance as parent-child nodes in the decision trees of the Random Forests.  Finally, using
...
% The proposed method is described in Figure \ref{figure:RF_Method}. First, a Random Forests model was created using all avaiable data. Then for each pair 
...
% based on our observations from using Random Forests. Using different simulation data, we observed that ``if two SNPs are
% interacting, they appear as parent-child nodes in a decision tree of a Random Forests more frequently than if they are
% not interacting''.  So, we utilised this observation to narrow down the vast search space of pairwise tests to detect
% interactions. First, we build a Random Forests and for each pair of SNPs we count the total number of times any SNP pair
...
% SNP pairs based on the number of times they appear as parent-child in the Random Forests decision trees. Higher ranking,
% represented by a smaller number, means a higher count. For example, a value of 1 for pair P3 means that SNP pair 3
% appeared as parent-child more than any other SNP pair in that Random Forests. Since in the base model three pairs of
% interacting SNPs do not have any marginal effects or any interaction effects, they did not appear as parent-child in any
% decision tree of the Random Forests. In the no interaction model, the three interacting SNP pairs appeared as
...
% In the proposed two step method for detecting SNP-SNP interactions, a random forests model was built and the count
...
% the SNP pairs appeared as parent-child in the decision trees of the Random Forests for at least 2 times, we selected them
...
% each Random Forests at different heritabilities (h = 0.33, 0.50, 0.66, and 1.00) for 10,000 SNP with 10 QTL and 3 pairs
...
% tests) and 4,798 individuals.  Using Random Forests, we have selected only 654 SNP pairs (out of more than 4.46 billion
...
% genomic data. We used a Random Forests algorithm to select a subset of SNP pairs from a vast search space so that the
